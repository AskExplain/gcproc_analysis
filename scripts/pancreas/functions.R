...
# Running all batch correction methods.
{
    ...
    mat <- list(uncorrected=t(uncorrected), 
                MNN=reducedDim(Xmnn),
                limma=t(Xlm),
                ComBat=t(Xcom)
                )
    ...
    if (length(batches)==2L) {
        colnames(batches[[1]]) <- paste0("Cell", c(1:dim(batches[[1]])[2]), "-1")
        colnames(batches[[2]]) <- paste0("Cell", c(1:dim(batches[[2]])[2]), "-2")
        rownames(batches[[1]]) <- rownames(batches[[2]]) <- paste0("Gene", c(1:dim(batches[[1]])[1]))
        ...
        print(Se)
        print(Se2)
        Y <- RunCCA(Se, Se2, do.normalize=FALSE, features = row.names(Se))
        mat$CCA <- Y@reductions$cca@cell.embeddings
    }
