...
s.c <- t(do.call('cbind',lapply(Z_segerstolpe_data,function(X){X$ref_y_covariates})))

x <- (do.call('cbind',lapply(Z_xin_data,function(X){X$ref_y})))
x.c <- t(do.call('cbind',lapply(Z_xin_data,function(X){X$ref_y_covariates})))

b <- log2(1+b*10000/colSums(b))
x <- log2(1+x*10000/colSums(x))
s <- log2(1+s*10000/colSums(s))

x <- t(t(x) - x.c%*%MASS::ginv(t(x.c)%*%(x.c))%*%t(x.c)%*%t(x))
s <- t(t(s) - s.c%*%MASS::ginv(t(s.c)%*%(s.c))%*%t(s.c)%*%t(s))


save(list = c("b","x","s"),file="./data/workflow/load_pancreas_clean_data.Rdata")
