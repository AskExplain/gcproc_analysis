gene_data <- read.csv("./main_image_as_gene_v1.csv",header=T)
cell_labels <- read.csv("./main_kidney_list.csv",header=T)
gene_type <- gene_data[,1]
cell_labels <- cell_labels[gene_type,2]
gene_data <- t(gene_data[,-1])
gene2pixel <- read.csv("./gene2pixel.csv",header=T,row.names=1)
